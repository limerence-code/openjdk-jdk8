...
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    libjvm = dlopen(jvmpath, RTLD_NOW + RTLD_GLOBAL); //加载libjvm.so库文件，目的是验证以下三个方法，后面要用
    if (libjvm == NULL) {
    ...
    ifn->CreateJavaVM = (CreateJavaVM_t)
        dlsym(libjvm, "JNI_CreateJavaVM");     //主要是验证以下三大方法可用性。--JNI_CreateJavaVM
...
/*//阻塞当前线程并开启一个新线程继续执行，主要通过操作系统函数pthread_create和pthread_join实现创建和阻塞
