...
)
{
     ...
     *     bugid 5030265 below.)
     */
    SelectVersion(argc, argv, &main_class); //从jar包中manifest文件或者命令行读取用户使用的JDK版本，判断当前版本是否合适
    ...
    if (!LoadJavaVM(jvmpath, &ifn)) {  //开始加载虚拟机（主要加载libjvm.so库文件并验证相关方法是否可用）
        return(6);
    }
    ...
    if (IsJavaArgs()) {
        ...
        cpath = getenv("CLASSPATH");   //设置classpath（从环境变量里获取）
        if (cpath == NULL) {
            cpath = ".";
        }
        SetClassPath(cpath);
     ...
     */
    if (!ParseArguments(&argc, &argv, &mode, &what, &ret, jrepath))   //解析命令行参数，如-version，-help等参数在该方法中解析的
    {
        return(ret);
    }

    /* Override class path if -jar flag was specified */ //以下对-jar等参数的特殊处理
    if (mode == LM_JAR) {
        SetClassPath(what);     /* Override class path */  
    }

    /* set the -Dsun.java.command pseudo property */
    SetJavaCommandLineProp(what, argc, argv); //解析形如-Dsun.java.command=的命令行参数
    ...
    return JVMInit(&ifn, threadStackSize, argc, argv, mode, what, ret);//通过JVMInit->ContinueInNewThread->ContinueInNewThread0->pthread_create创建了一个新的线程，执行JavaMain函数，主线程pthread_join该线程，在JavaMain函数中完成虚拟机的初始化和启动。
}
/*
...
InitializeJVM(JavaVM **pvm, JNIEnv **penv, InvocationFunctions *ifn)
{
    ...
    r = ifn->CreateJavaVM(pvm, (void **)penv, &args);  //这里开始创建虚拟机调用的是jni.cpp文件中的JNI_CreateJavaVM
    JLI_MemFree(options);
    return r == JNI_OK;
}
