...
/*
 * This file contains the main entry point into the launcher code    //main函数进入启动器代码的入口
 * this is the only file which will be repeatedly compiled by other  //唯一一个可以被其他工具重复编译的文件
 * tools. The rest of the files will be linked in.   //其他文件都会和它有关联
 */
...
#else /* JAVAW */   //win下的宏定义。略过
int
main(int argc, char **argv)    //main函数入口，虚拟机启动入口
{
    int margc;
    char** margv;
