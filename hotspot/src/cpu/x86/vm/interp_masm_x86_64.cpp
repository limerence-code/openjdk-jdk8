...
//进入此方法目标obj要么是无锁状态，要么是对同一个对象的synchronized嵌套情形下的有锁状态
    ...
    if (UseBiasedLocking) {//UseBiasedLocking默认为true, 首先尝试获取偏向锁，获取成功会跳转到done，否则走到slow_case:标记,类似于goto,这里指的是InterpreterRuntime::monitorenter()
      biased_locking_enter(lock_reg, obj_reg, swap_reg, rscratch1, false, done, &slow_case);//偏向锁申请。lock_reg:存储指向BasicObjectLock的指针；obj_reg:存储锁对象的指针;done:标记,标志着获取锁成功。
    }

    // Load immediate 1 into swap_reg %rax
    movl(swap_reg, 1);//如果UseBiasedLocking为false或者目标对象的锁不是偏向锁了会走此逻辑

    // Load (object->mark() | 1) into swap_reg %rax
    orptr(swap_reg, Address(obj_reg, 0)); //计算 object->mark() | 1，结果保存到swap_reg，跟1做或运算将其标记为无锁状态

    // Save (object->mark() | 1) into BasicLock's displaced header
    movptr(Address(lock_reg, mark_offset), swap_reg); //将(object->mark() | 1)的结果保存到BasicLock的displaced_header中，保存原来的对象头

    assert(lock_offset == 0,
           "displached header must be first word in BasicObjectLock");//lock_reg即是里面的lock属性的地址
    ...
    if (PrintBiasedLockingStatistics) {
      cond_inc32(Assembler::zero,
                 ExternalAddress((address) BiasedLocking::fast_path_entry_count_addr()));//增加计数器
    }
    jcc(Assembler::zero, done); //如果等于，说明obj的对象头是无锁状态的，此时跟1做或运算，结果不变

    // Test if the oopMark is an obvious stack pointer, i.e., //如果不等于，说明obj的对象头要么是偏向锁，要么是重量级锁，多线程下可能其他线程已经获取了该对象的轻量级锁
    //  1) (mark & 7) == 0, and                               //左边的汇编指令相当于执行如下判断，判断目标对应的对象头是否属于当前调用栈帧，如果是说明还是当前线程占有该轻量级锁，如果不是则说明其他线程占用了轻量级锁或者已经膨胀成重量级锁
    ...
    // Save the test result, for recursive case, the result is zero //在递归即synchronized嵌套使用的情形下，上述指令计算的结果就是0
    movptr(Address(lock_reg, mark_offset), swap_reg);  //当BasicLock的displaced_header置为NULL

    if (PrintBiasedLockingStatistics) {
      cond_inc32(Assembler::zero,
                 ExternalAddress((address) BiasedLocking::fast_path_entry_count_addr()));//增加计数器
    }
    jcc(Assembler::zero, done);  //如果andptr的结果为0，说明当前线程已经获取了轻量级锁则跳转到done

    bind(slow_case); //否则执行InterpreterRuntime::monitorenter将轻量级锁膨胀成重量级锁或者获取重量级锁
    ...
    bind(done);// 这里的done和上面传入到偏向锁的done是一样的。直接跳到这表明获取锁成功，接下来就会返回进行字节码的执行了。
  }
}
