...
int MacroAssembler::biased_locking_enter(Register lock_reg,
                                         ...
                                         Label& done,
                                         Label* slow_case,
                                         BiasedLockingCounters* counters) {  //获取偏向锁入口 32位版本
  ...
  // Biased locking                  1、首先判断 Mark Word 中的后三位（是否偏向锁+锁标志位）的值是否为5(101)，即是否为偏向
  // See whether the lock is currently biased toward our thread and
  // whether the epoch is still valid                  锁状态。如果是则执行后面 fast_enter 的逻辑，如果不是则执行第2步
  // Note that the runtime guarantees sufficient alignment of JavaThread
  // pointers to allow age to be placed into low bits
  // First check to see whether biasing is even enabled for this object
  Label cas_label;
  int null_check_offset = -1;
  if (!swap_reg_contains_mark) {// 如果 swap_reg 中没存 mark_addr，那么就先将 mark_addr 存入 swap_reg 中
  ...
  movl(tmp_reg, swap_reg);//将对象的 mark_addr，即 markOop 指针移入 tmp_reg 中
  andl(tmp_reg, markOopDesc::biased_lock_mask_in_place);// 将 tmp_reg 和 biased_lock_mask_in_place(111) 进行与操作,取出markOop中后三位，即(是否偏向锁+锁标志位)
  cmpl(tmp_reg, markOopDesc::biased_lock_pattern);// 查看 Mark Word 后三位是否为5(biased_lock_pattern 为5, 即101）,如果不相等，则表明不为偏向锁状态,跳往 cas_label. 否则即为偏向锁状态
  if (need_tmp_reg) {
    pop(tmp_reg);
  }
...
int MacroAssembler::biased_locking_enter(Register lock_reg,
                                         ...
                                         Label& done,
                                         Label* slow_case,
                                         BiasedLockingCounters* counters) {  //获取偏向锁入口 64位
  assert(UseBiasedLocking, "why call this otherwise?");
  ...
  movq(tmp_reg, swap_reg);//将对象的 mark_addr，即 markOop 指针移入 tmp_reg 中
  andq(tmp_reg, markOopDesc::biased_lock_mask_in_place);// 将 tmp_reg 和 biased_lock_mask_in_place(111) 进行与操作,取出markOop中后三位，即(是否偏向锁+锁标志位)
  cmpq(tmp_reg, markOopDesc::biased_lock_pattern);// 查看 Mark Word 后三位是否为5(biased_lock_pattern 为5, 即101）,如果不相等，则表明不为偏向锁状态,跳往 cas_label. 否则即为偏向锁状态
  jcc(Assembler::notEqual, cas_label);
  // The bias pattern is present in the object's header. Need to check// 2、判断锁对象Mark Word中是否包含当前线程地址,最后三位标志位是否相同
  // whether the bias owner and the epoch are both still current.// 且epoch值和类的epoch值是否相等。如果都相同，那么当前线程持有该偏向锁，可以直接返回。不然执行第3步
  load_prototype_header(tmp_reg, obj_reg); // 将类的prototype_header(Mark Word)加载到tmp_reg中
  orq(tmp_reg, r15_thread);//将当前线程地址和类的的prototype_header相或，这样得到的结果为(当前线程id + prototype_header中的(epoch + 分代年龄 + 偏向锁标志 + 锁标志位))
  xorq(tmp_reg, swap_reg);// 将上面计算得到的结果与锁对象的markOop进行异或,得到一个结果diff(相等的位被置为0)
  andq(tmp_reg, ~((int) markOopDesc::age_mask_in_place));// 将header_reg中除了分代年龄之外的其他位取了出来，即将上面异或得到的结果中分代年龄给忽略掉。
  if (counters != NULL) {
    cond_inc32(Assembler::zero,
               ExternalAddress((address) counters->anonymously_biased_lock_entry_count_addr()));
  }
  jcc(Assembler::equal, done);//如果除了分代年龄，对象的markOop和(当前线程地址+其他位)相等，那么上面与操作的结果应该为0，表明对象之前已经偏向当前线程,那么跳到done处，执行同步代码块中的代码,否则表明当前线程还不是偏向锁的持有者，会接着往下走。

  Label try_revoke_bias;
  Label try_rebias;

  // At this point we know that the header has the bias pattern and// 3、判断类对象是否支持偏向锁，如果不支持，则跳转到第6步执行移除偏向锁的逻辑。
  // that we are not the bias owner in the current epoch. We need to// 如果支持则跳转到第4步执行
  // figure out more details about the state of the header in order to// header_reg中存储的是(当前线程id + prototype_header中的(epoch + 分代年龄 + 偏向锁标志 +
  // know what operations can be legally performed on the object's// 锁标志位)) 和 锁对象 Mark Word异或的结果，我们要查看后三位(biased_lock_mask_in_place的
  // header.// 值是111)的结果是否为0，如果不为0，表示之前异或时锁对象的Mark Word后三位和对象所属类的后三位不一致,所以对象所属类不再支持偏向锁，
// 此时需要跳转到try_revoke_bias进行移除偏向锁操作。
  // If the low three bits in the xor result aren't clear, that means
  // the prototype header is no longer biased and we have to revoke
  // the bias on this object.
  testq(tmp_reg, markOopDesc::biased_lock_mask_in_place);//这个testptr的实现实际上是获取第一个参数多少位的值。多少位是根据第二个参数的二进制长度来决定的。
  jcc(Assembler::notZero, try_revoke_bias);

  // Biasing is still enabled for this data type. See whether the
  // epoch of the current bias is still valid, meaning that the epoch
  // bits of the mark word are equal to the epoch bits of the
  // prototype header. (Note that the prototype header's epoch bits// 4、执行到这里表示锁对象以及类对象都支持偏向锁,但是并不是偏向的当前线程。
  // only change at a safepoint.) If not, attempt to rebias the object// 所以接下来会判断异或结果中的epoch是否为0，如果为0，则跳转到第5步执行。
  // toward the current thread. Note that we must be absolutely sure// 如果不为0，则证明锁过期了,跳转到第7步执行重新偏向逻辑
  // that the current epoch is invalid in order to do this because
  // otherwise the manipulations it performs on the mark word are
  // illegal.
  testq(tmp_reg, markOopDesc::epoch_mask_in_place);//测试锁对象的epoch值和锁对象类的epoch是否相等，如果不相等，则证明锁过期了，需要重新偏向
  jcc(Assembler::notZero, try_rebias);
  ...
  // fails we will go in to the runtime to revoke the object's bias.
  // Note that we first construct the presumed unbiased header so we
  // don't accidentally blow away another thread's valid bias.// 5、表明锁对象还未偏向任何线程，则可以尝试去获取锁，使得对象偏向当前线程
  andq(swap_reg,
       markOopDesc::biased_lock_mask_in_place | markOopDesc::age_mask_in_place | markOopDesc::epoch_mask_in_place);//取出对象Mark Word中除线程地址之外的其他位
  movq(tmp_reg, swap_reg); // 将其他位移动至 tmp_reg
  orq(tmp_reg, r15_thread);// 将其他位和当前线程进行或，构造成一个新的完整的Mark Word,存入tmp_reg中。新的Mark Word因为保存了当前线程地址，所以会偏向当前线程。
  if (os::is_MP()) {
    lock();
  }
  cmpxchgq(tmp_reg, Address(obj_reg, 0));// 尝试利用CAS操作将新构成的Mark Word存入锁对象的mark_addr(Mark Word)，如果设置成功，则获取偏向锁成功。
  // If the biasing toward our thread failed, this means that//cmpxchgptr操作会强制将rax寄存器（swap_reg）中内容作为老数据，与第二个参数，
  // another thread succeeded in biasing it toward itself and we//在这里即mark_addr处的内容进行比较，如果相等，则将第一个参数的内容，即tmp_reg
  // need to revoke that bias. The revocation will occur in the//中的新数据，存入mark_addr。
  // interpreter runtime in the slow case.
  if (counters != NULL) {
    cond_inc32(Assembler::zero,
               ExternalAddress((address) counters->anonymously_biased_lock_entry_count_addr()));
  }
  if (slow_case != NULL) {// 上面CAS操作失败的情况下，表明对象头中的markOop数据已经被篡改，即有其他线程已经获取到偏向锁，因为偏向锁不容许多个线程访问同一个锁对象，所以需要跳到
    jcc(Assembler::notZero, *slow_case);//slow_case(InterpreterRuntime::monitorenter)处，去撤销该对象的偏向锁，并进行锁升级
  }
  jmp(done);//上面CAS成功的情况下，直接就跳往done处，回去执行方法的字节码了

  bind(try_rebias);//6、try_revoke_bias使用CAS操作，重置mark word。撤销偏向锁后后续所有操作都走轻量级锁的加锁过程
  // At this point we know the epoch has expired, meaning that the//try_revoke_bias和try_rebias的代码定义也在biased_locking_enter中
  // current "bias owner", if any, is actually invalid. Under these
  // circumstances _only_, we are allowed to use the current header's
  // value as the comparison value when doing the cas to acquire the
  // bias in the current epoch. In other words, we allow transfer of
  // the bias from one thread to another directly in this situation.// 类的prototype_header中已经支持偏向锁的位了，即这个类的所有对象都不再支持偏
  //                                   //向锁了，但是当前对象仍为偏向锁状态，所以我们需要重置下当前对象的markOop为无锁态。
  // FIXME: due to a lack of registers we currently blow away the age
  // bits in this situation. Should attempt to preserve them.
  load_prototype_header(tmp_reg, obj_reg);// 将锁对象所属类的prototype_header送入tmp_reg。
  orq(tmp_reg, r15_thread);
  if (os::is_MP()) {
    lock();
  }
  cmpxchgq(tmp_reg, Address(obj_reg, 0));// 尝试用CAS操作，使对象的markOop重置为无锁态。这里是否失败无所谓，即使失败
  // If the biasing toward our thread failed, then another thread//了，也表明其他线程已经移除了对象的偏向锁标志。
  // succeeded in biasing it toward itself and we need to revoke that
  // bias. The revocation will occur in the runtime in the slow case.
  if (counters != NULL) {
    cond_inc32(Assembler::zero,
  ...
  bind(try_revoke_bias);//7、try_rebias就是将使得锁对象重新偏向当前线程，如果失败则走slow_case(InterpreterRuntime::monitorenter)进行偏向锁撤销逻辑
  // The prototype mark in the klass doesn't have the bias bit set any
  // more, indicating that objects of this data type are not supposed
  // to be biased any more. We are going to try to reset the mark of
  // this object to the prototype value and fall through to the
  ...
  cmpxchgq(tmp_reg, Address(obj_reg, 0));// 尝试用CAS操作，成功表示重偏向加锁成功。如果CAS失败，则表明Mark Word已经被其他线程更改，需要跳往slow_case进行撤销偏向锁，否则跳往done处，执行字节码
  // Fall through to the normal CAS-based lock, because no matter what
  // the result of the above CAS, some thread must have succeeded in
  // removing the bias bit from the object's header.
  if (counters != NULL) {
