...
UNSAFE_ENTRY(void, Unsafe_SetObjectVolatile(JNIEnv *env, jobject unsafe, jobject obj, jlong offset, jobject x_h))
  ...
  oop p = JNIHandles::resolve(obj);
  void* addr = index_oop_from_field_offset_long(p, offset);
  OrderAccess::release();//storestore
  if (UseCompressedOops) {
    oop_store((narrowOop*)addr, x);
  } else {
    oop_store((oop*)addr, x);
  }
  OrderAccess::fence();//storeload
UNSAFE_END
...
UNSAFE_ENTRY(jboolean, Unsafe_CompareAndSwapInt(JNIEnv *env, jobject unsafe, jobject obj, jlong offset, jint e, jint x))
  ...
  return (jint)(Atomic::cmpxchg(x, addr, e)) == e;//关键代码 x：希望修改的值，addr：希望对这个地址里面的值进行修改，e：期望值（addr里面原来的值）
UNSAFE_END
...
UNSAFE_ENTRY(void, Unsafe_Park(JNIEnv *env, jobject unsafe, jboolean isAbsolute, jlong time))
  ...
  thread->parker()->park(isAbsolute != 0, time);  //获取java线程的parker，然后执行它的park方法
#ifndef USDT2
