...
JVM_ENTRY_NO_ENV(jlong, JVM_MaxMemory(void))
  ...
  size_t n = Universe::heap()->max_capacity(); //root/jdk/hotspot/src/share/vm/memory/genCollectedHeap.cpp:253
  return convert_size_t_to_jlong(n);
JVM_END
...
JVM_ENTRY(void, JVM_MonitorWait(JNIEnv* env, jobject handle, jlong ms))
  ...
  ObjectSynchronizer::wait(obj, ms, CHECK);//这里是重点
JVM_END
...
JVM_ENTRY(void, JVM_MonitorNotify(JNIEnv* env, jobject handle))
  ...
  Handle obj(THREAD, JNIHandles::resolve_non_null(handle));
  ObjectSynchronizer::notify(obj, CHECK);//重点
JVM_END
...
static void thread_entry(JavaThread* thread, TRAPS) {  //这里就是反向调用java中Thread类的run方法入口
  HandleMark hm(THREAD);
...
JVM_ENTRY(void, JVM_StartThread(JNIEnv* env, jobject jthread))
  ...
  // in Thread::start.
  {
    ...
    if (java_lang_Thread::thread(JNIHandles::resolve_non_null(jthread)) != NULL) {
      throw_illegal_thread_state = true; //如果关联的JavaThread不为空，说明该线程已经启动过，需要抛出异常
    } else {
      ...
      if (native_thread->osthread() != NULL) {//osthread不为空，prepare方法会建立Java Thread实例同JavaThread之间的关联关系
        // Note: the current thread is not being used within "prepare".
        native_thread->prepare(jthread);     //并将Java线程的优先级映射成原生线程的优先级并设置（当前线程会加入链表）
      }
    }
  }

  if (throw_illegal_thread_state) {     //关联的JavaThread不为空，抛出异常
    THROW(vmSymbols::java_lang_IllegalThreadStateException());
  }

  assert(native_thread != NULL, "Starting null thread?");

  if (native_thread->osthread() == NULL) {  //创建osthread失败，抛出异常
...
JVM_ENTRY(jboolean, JVM_IsThreadAlive(JNIEnv* env, jobject jthread))
  ...
  oop thread_oop = JNIHandles::resolve_non_null(jthread); //从JNI引用中解析出对应的Thread实例oop,比如调用join的线程
...
JVM_ENTRY(void, JVM_SetThreadPriority(JNIEnv* env, jobject jthread, jint prio))
  ...
  JavaThread* thr = java_lang_Thread::thread(java_thread); //获取关联的JavaThread实例
...
JVM_ENTRY(void, JVM_Yield(JNIEnv *env, jclass threadClass))//用于让出当前线程的CPU时间片
  ...
  if (ConvertYieldToSleep) {//ConvertYieldToSleep默认为false
...
JVM_ENTRY(void, JVM_Sleep(JNIEnv* env, jclass threadClass, jlong millis))
  ...
  if (millis < 0) {  //如果睡眠的时间小于0，则抛出异常。这里数据的校验在jvm层逻辑中校验
    THROW_MSG(vmSymbols::java_lang_IllegalArgumentException(), "timeout value is negative");
  }
  //如果线程已经被打断了，那么也抛出异常
  if (Thread::is_interrupted (THREAD, true) && !HAS_PENDING_EXCEPTION) { //如果已经被中断则抛出异常
    THROW_MSG(vmSymbols::java_lang_InterruptedException(), "sleep interrupted");
  }

  // Save current thread state and restore it at the end of this block.
  // And set new thread state to SLEEPING.
  JavaThreadSleepState jtss(thread); //增加线程sleep的次数，并启动计入sleep耗时的计时器
  ...
  if (millis == 0) { //这里允许睡眠时间为0
    ...
    if (ConvertSleepToYield) {//x86下ConvertSleepToYield为true
      ...
      thread->osthread()->set_state(SLEEPING);//将osthread的状态置为SLEEPING
      os::sleep(thread, MinSleepInterval, false);//调用系统级别的sleep方法，此时只会睡眠最小时间间隔
      thread->osthread()->set_state(old_state); //恢复线程的状态
    }
  } else {//休眠时间大于0
    ThreadState old_state = thread->osthread()->get_state();//获取并保存线程的旧状态
    thread->osthread()->set_state(SLEEPING);//将osthread的状态置为SLEEPING
    if (os::sleep(thread, millis, true) == OS_INTRPT) {//重要的是这里调用os的sleep方法，这里还判断是不是被中断了
    ...
    thread->osthread()->set_state(old_state);//恢复原来的状态
  }
...
JVM_ENTRY(jobject, JVM_CurrentThread(JNIEnv* env, jclass threadClass))
  ...
  oop jthread = thread->threadObj();//thread是JVM_ENTRY中获取的，获取其关联的Thread实例oop
  assert (thread != NULL, "no current thread!");
  return JNIHandles::make_local(env, jthread); //返回该实例oop，注意需要将其包裹成JNI本地引用
JVM_END
...
JVM_QUICK_ENTRY(jboolean, JVM_IsInterrupted(JNIEnv* env, jobject jthread, jboolean clear_interrupted))
  ...
  if (thr == NULL) {
    return JNI_FALSE;
  } else {//JavaThread不为空，判断其是否被中断，clear_interrupted默认为true，此处就是1
...
JVM_ENTRY(jboolean, JVM_HoldsLock(JNIEnv* env, jclass threadClass, jobject obj))
  ...
  if (obj == NULL) {
    THROW_(vmSymbols::java_lang_NullPointerException(), JNI_FALSE);//obj为空抛出异常
  }
...
JVM_ENTRY(jstring, JVM_InternString(JNIEnv *env, jstring str))//str其实就是调用intern方法的string，只不过这里的str实际上是个句柄
