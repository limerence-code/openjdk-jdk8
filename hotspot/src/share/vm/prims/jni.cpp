...
static void jni_invoke_static(JNIEnv *env, JavaValue* result, jobject receiver, JNICallType call_type, jmethodID method_id, JNI_ArgumentPusher *args, TRAPS) {
  ...
  JavaCalls::call(result, method, &java_args, CHECK); //凡是看到JavaCalls::call就是反向调用java中的方法
...
JNI_ENTRY(void, jni_CallStaticVoidMethod(JNIEnv *env, jclass cls, jmethodID methodID, ...))
  ...
  JavaValue jvalue(T_VOID);
  JNI_ArgumentPusherVaArg ap(methodID, args);
  jni_invoke_static(env, &jvalue, NULL, JNI_STATIC, methodID, &ap, CHECK);  //真正在这里调的
  va_end(args);
JNI_END
...
#endif // INCLUDE_JNI_CHECK
  return &jni_NativeInterface; //返回一个全局变量
}
...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
  ...
  if (Atomic::xchg(1, &vm_created) == 1) {//通过Atomic::xchg方法修改全局volatile变量vm_created为1，该变量默认为0，如果返回1则说明JVM已经创建完成或者创建中，返回JNI_EEXIST错误码，如果返回0则说明JVM未创建
    return JNI_EEXIST;   // already created, or create attempt in progress
  }
  if (Atomic::xchg(0, &safe_to_recreate_vm) == 0) {//通过Atomic::xchg方法修改全局volatile变量safe_to_recreate_vm为0，该变量默认为1，如果返回0则说明JVM已经在重新创建了，返回JNI_ERR错误码，如果返回1则说明JVM未创建
   ...
   */
  bool can_try_again = true;//完成JVM的初始化，如果初始化过程中出现不可恢复的异常则can_try_again会被置为false

  result = Threads::create_vm((JavaVMInitArgs*) args, &can_try_again);  //这里创建虚拟机入口
  if (result == JNI_OK) {  //初始化正常
    JavaThread *thread = JavaThread::current();//获取当前线程，即执行create_vm的线程，也是JNI_CreateJavaVM执行完毕后执行main方法的线程
    /* thread is thread_in_vm here */
    *vm = (JavaVM *)(&main_vm);/*JavaVM赋值，main_vm是jni.h中的全局变量，最终实现指向全局变量jni_InvokeInterface */
    *(JNIEnv**)penv = thread->jni_environment();//JNIEnv赋值

    // Tracks the time application was running before GC
    RuntimeService::record_application_start();// 记录应用的启动时间

    // Notify JVMTI
    if (JvmtiExport::should_post_thread_life()) {// 通知JVMTI应用启动
    ...
    if (CompileTheWorld) ClassLoader::compile_the_world();//根据配置加载类路径中所有的类
    if (ReplayCompiles) ciReplay::replay(thread);

    // Some platforms (like Win*) need a wrapper around these test
    // functions in order to properly handle error conditions.
    CALL_TEST_FUNC_WITH_WRAPPER_IF_NEEDED(test_error_handler);// win* 系统添加异常处理的wrapper
    CALL_TEST_FUNC_WITH_WRAPPER_IF_NEEDED(execute_internal_vm_tests);
#endif

    // Since this is not a JVM_ENTRY we have to set the thread state manually before leaving.
    ThreadStateTransition::transition_and_fence(thread, _thread_in_vm, _thread_in_native);//设置当前线程的线程状态
  } else {    //如果create_vm初始化失败
    if (can_try_again) {// 如果可以重试则恢复默认值1
      // reset safe_to_recreate_vm to 1 so that retrial would be possible
      safe_to_recreate_vm = 1;
    }

    // Creation failed. We must reset vm_created  
    *vm = 0;                     //将vm和penv置空，vm_created重置成0
    *(JNIEnv**)penv = 0;
...
static jint attach_current_thread(JavaVM *vm, void **penv, void *_args, bool daemon) {
  ...
  */
  ...
  JavaThread* thread = new JavaThread(true); //新建java线程
  ...
  { Threads_lock->lock_without_safepoint_check();
    ...
    Threads::add(thread, daemon);   //线程加入到链表结构中
    Threads_lock->unlock();
  }
...
jint JNICALL jni_DetachCurrentThread(JavaVM *vm)  { //线程退出时调用
#ifndef USDT2
  ...
  // maintenance work?)
  thread->exit(false, JavaThread::jni_detach);   //分离线程后退出
  delete thread;
