...
JavaThread* java_lang_Thread::thread(oop java_thread) {//在调用ObjectLocker的notify_all方法前会将该属性置为NULL
  return (JavaThread*)java_thread->address_field(_eetop_offset); //获取eetop属性，该属性是一个long类型，保存关联的JavaThread指针
}


void java_lang_Thread::set_thread(oop java_thread, JavaThread* thread) {
  java_thread->address_field_put(_eetop_offset, (address)thread);//设置eetop属性
}
...
ThreadPriority java_lang_Thread::priority(oop java_thread) {
  return (ThreadPriority)java_thread->int_field(_priority_offset);//获取priority属性，默认是继承父线程的的线程优先级
}
...
bool java_lang_Thread::is_stillborn(oop java_thread) { //stillborn表示线程是否被中止，调用stop方法时会将该属性置为true
...
bool java_lang_Thread::is_alive(oop java_thread) {//获取关联的JavaThread，如果为NULL表示该线程已经退出
...
jlong java_lang_Thread::stackSize(oop java_thread) {
  // The stackSize field is only present starting in 1.4 //获取stackSize属性，只有1.4才会读取
