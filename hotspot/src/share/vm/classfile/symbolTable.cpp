...
oop StringTable::intern(Handle string_or_null, jchar* name,
                        int len, TRAPS) {
  unsigned int hashValue = hash_string(name, len);//获取string的hash值
  int index = the_table()->hash_to_index(hashValue);
  oop found_string = the_table()->lookup(index, name, len, hashValue);//根据索引和char数组以及数组长度去stringTable里面查找有没有指向的内存地址的内容和char数组内容相同的指针

  // Found
  if (found_string != NULL) return found_string;//如果找到了指向的内容和char数组内容相同的指针，直接返回
  ...
  } else {// 如果在stringTable中不存在指向的内容和char数组内容相同的指针，那么就构建一个string的obj，并用句柄关联
  ...
  return the_table()->basic_add(index, string, name, len,
                                hashValue, CHECK_NULL);// 将构建的string的obj加到stringTable，并返回
}
