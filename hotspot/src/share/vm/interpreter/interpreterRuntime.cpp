...
// be shared by method invocation and synchronized blocks. //synchronized修饰方法和代码块公用此方法
//%note synchronization_3

//%note monitor_1
IRT_ENTRY_NO_ASYNC(void, InterpreterRuntime::monitorenter(JavaThread* thread, BasicObjectLock* elem)) //用于获取轻量级锁或者重量级锁
#ifdef ASSERT
  ...
  if (UseBiasedLocking) {//如果开启了偏向锁功能,1.6之后默认开启，如果确定线程有激烈竞争，可以直接取消偏向锁，使用参数-XX:UseBiasedLocking=false
    // Retry fast entry if bias is revoked to avoid unnecessary inflation
    ObjectSynchronizer::fast_enter(h_obj, elem->lock(), true, CHECK);//走快速enter可以避免不必要膨胀为重量级锁
  } else {  //当关闭偏向锁功能，或多个线程竞争偏向锁导致偏向锁升级为轻量级锁，会尝试获取轻量级锁，这是入口
...
IRT_ENTRY_NO_ASYNC(void, InterpreterRuntime::monitorexit(JavaThread* thread, BasicObjectLock* elem))
  ...
  assert(Universe::heap()->is_in_reserved_or_null(h_obj()),
         "must be NULL or an object");
  if (elem == NULL || h_obj()->is_unlocked()) {  //如果BasicObjectLock为空或者目标对象没有持有锁则抛出异常
    THROW(vmSymbols::java_lang_IllegalMonitorStateException());
  }
  ObjectSynchronizer::slow_exit(h_obj(), elem->lock(), thread);//轻量级锁和重量级锁的释放
