...
#define ARRAY_STOREFROM64(T, T2, stackSrc, extra)                                    \
      ...
      CASE(_monitorenter): {
        oop lockee = STACK_OBJECT(-1);//获取锁对象 //栈帧当中生成一个lock record 记录
        // derefing's lockee ought to provoke implicit null check
        CHECK_NULL(lockee);
        ...
        if (entry != NULL) {
          entry->set_obj(lockee);//将lock record中的obj指向锁对象
          markOop displaced = lockee->mark()->set_unlocked();//获取锁对象头信息
          entry->lock()->set_displaced_header(displaced);
          if (Atomic::cmpxchg_ptr(entry, lockee->mark_addr(), displaced) != displaced) {
            // Is it simple recursive case?
            if (THREAD->is_lock_owned((address) displaced->clear_lock_bits())) {
      ...
      CASE(_putstatic):
        {
          ...
          //
          int field_offset = cache->f2_as_index();
          if (cache->is_volatile()) {//这里就是判断字段是否加volatile
            if (tos_type == itos) {
            ...
            OrderAccess::storeload();//volatile关键字加上storeload屏障
          } else {
            if (tos_type == itos) {
