...
class BiasedLockingCounters VALUE_OBJ_CLASS_SPEC {  //跟偏向锁有关的统计属性
 private:
...
class BiasedLocking : AllStatic {  //偏向锁的逻辑都封装在BiasedLocking
private:
...
public:
  ...
  enum Condition {
    NOT_BIASED = 1, //对象没有持有偏向锁
    BIAS_REVOKED = 2, //该对象的偏向锁已经被撤销了,即其对象头已经恢复成默认的不开启偏向锁时的状态
    BIAS_REVOKED_AND_REBIASED = 3 //表示当前线程重新获取了该偏向锁
  };
  ...
  static void init();  //初始化BiasedLocking，该方法是在JVM启动时调用的(JNI_CreateJavaVM->Thread::create_vm->init)
