...
void ObjectSynchronizer::fast_enter(Handle obj, BasicLock* lock, bool attempt_rebias, TRAPS) { //obj是申请锁的对象，lock是线程栈帧中的lock record
 if (UseBiasedLocking) {  //检查是否启用了偏向锁，如果没有启动则直接执行slow_enter
    if (!SafepointSynchronize::is_at_safepoint()) { //如果不在安全点（Stop-the-World暂停机制，java代码都会挂起），则获取偏向锁
      BiasedLocking::Condition cond = BiasedLocking::revoke_and_rebias(obj, attempt_rebias, THREAD);//获取偏向锁(特殊情况会重新获取偏向锁)
      if (cond == BiasedLocking::BIAS_REVOKED_AND_REBIASED) {//这里就是判断偏向锁标志位1，锁标记为01，如果不是，转轻量级锁
        return; //撤销完返回
      }
 ...
 }//以上步骤是要把偏向锁撤销，只有撤销掉才能走轻量级锁

 slow_enter (obj, lock, THREAD) ;//走轻量级锁模式
}

void ObjectSynchronizer::fast_exit(oop object, BasicLock* lock, TRAPS) {  //轻量级锁的释放
  assert(!object->mark()->has_bias_pattern(), "should not see bias pattern here");
  // if displaced header is null, the previous enter is recursive enter, no-op
  markOop dhw = lock->displaced_header();
  markOop mark ;
  if (dhw == NULL) {//对同一目标对象的synchronized嵌套情形，该对象的原来的对象头保存在外层synchronized对应的BasicLock或者ObjectMonitor中，外层synchronized解锁时会恢复其对象头
     // Recursive stack-lock.
     // Diagnostics -- Could be: stack-locked, inflating, inflated.
     mark = object->mark() ;
     assert (!mark->is_neutral(), "invariant") ;
     if (mark->has_locker() && mark != markOopDesc::INFLATING()) {//如果持有轻量级锁
        assert(THREAD->is_lock_owned((address)mark->locker()), "invariant") ;
     }
     if (mark->has_monitor()) {//如果持有重量级锁
  ...
  if (mark == (markOop) lock) {
     assert (dhw->is_neutral(), "invariant") ;//持有轻量级锁，dhw保存着原来的对象头
     if ((markOop) Atomic::cmpxchg_ptr (dhw, object->mark_addr(), mark) == mark) {//恢复对象的对象头
        TEVENT (fast_exit: release stacklock) ;
        return;
     }           //此时持有该对象轻量级锁的只有当前线程，所以原子修改不会失败
  }

  ObjectSynchronizer::inflate(THREAD, object)->exit (true, THREAD) ;//如果持有重量级锁，则重量锁解锁
}
...
void ObjectSynchronizer::slow_enter(Handle obj, BasicLock* lock, TRAPS) {  //获取轻量级锁入口
  markOop mark = obj->mark(); //先获取mark word
  assert(!mark->has_bias_pattern(), "should not see bias pattern here"); //此时不应该是偏向状态

  if (mark->is_neutral()) { //如果此时是无锁状态
    // Anticipate successful CAS -- the ST of the displaced mark must
    // be visible <= the ST performed by the CAS.
    lock->set_displaced_header(mark); //设置头信息：虚拟机首先将在当前线程的栈帧中建立一个名为锁记录（Lock Record）的空间，用于存储锁对象Mark Word的拷贝
    if (mark == (markOop) Atomic::cmpxchg_ptr(lock, obj()->mark_addr(), mark)) { //如果是没有锁的，就把线程帧栈里的Lock Record设置为那个Mark Work。然后让mark指向那个lock
      TEVENT (slow_enter: release stacklock) ; //将lock作为原来的对象头，因为BasicLock本身的大小就是8字节的，所以lock地址的后3位都是0,0就表示轻量级锁
      return ;                                //如果设置成功表示已经获取轻量级锁，返回
    }
    // Fall through to inflate() ... 这里cas获取轻量级锁失败后直接膨胀进入258行
  } else   //如果对象已经被锁了，看一下是不是就是请求锁的线程锁的，如果是就万事大吉
  ...
  lock->set_displaced_header(markOopDesc::unused_mark());  //因为ObjectMonitor中会保存对象的对象头，所以此处不需要在保存了
  ObjectSynchronizer::inflate(THREAD, obj())->enter(THREAD); //如果还不是的话就要做锁膨胀了，真的monitor就要出来了
}
...
ObjectLocker::~ObjectLocker() {//析构函数（一般用来和构造函数执行相反的动作）
  if (_dolock) {
    ObjectSynchronizer::fast_exit(_obj(), &_lock, _thread);//析构函数释放锁
  }
}
...
void ObjectSynchronizer::wait(Handle obj, jlong millis, TRAPS) {
  ...
  ObjectMonitor* monitor = ObjectSynchronizer::inflate(THREAD, obj()); //获取Object的Monitor对象（即内置锁）
  DTRACE_MONITOR_WAIT_PROBE(monitor, obj(), THREAD, millis);
  monitor->wait(millis, true, THREAD);                         //调用monitor的wait方法
...
intptr_t ObjectSynchronizer::FastHashCode (Thread * Self, oop obj) {
  if (UseBiasedLocking) {//如果启用了偏向锁
    ...
    if (obj->mark()->has_bias_pattern()) { //如果此时正处于偏向状态
      ...
      BiasedLocking::revoke_and_rebias(hobj, false, JavaThread::current()); //撤销偏向锁
      obj = hobj() ;
      assert(!obj->mark()->has_bias_pattern(), "biases should be revoked by now");//看看有没有成功撤销
    }
  }
  ...
  markOop temp, test;
  intptr_t hash;
  markOop mark = ReadStableMark (obj);//读出一个稳定的markword，防止obj正处于锁膨胀状态，如果正在膨胀，那么就等他膨胀完，再读出来

  // object should remain ineligible for biased locking
  assert (!mark->has_bias_pattern(), "invariant") ;

  if (mark->is_neutral()) { //如果是无锁状态，则获取hashcode
    hash = mark->hash();              // this is a normal header
    if (hash) {                       // if it has hash, just return it 如果有hashcode值，则返回
      return hash;
    }
    hash = get_next_hash(Self, obj);  // allocate a new hash code  如果没有，则通过get_next_hash给你算出一个
  ...
  } else if (mark->has_monitor()) { //处于重量级锁状态
  ...
  } else if (Self->is_lock_owned((address)mark->locker())) { //轻量级锁状态
  ...
  monitor = ObjectSynchronizer::inflate(Self, obj); //膨胀过程后返回monitor
...
bool ObjectSynchronizer::current_thread_holds_lock(JavaThread* thread,
                                                   Handle h_obj) {
  if (UseBiasedLocking) {
    BiasedLocking::revoke_and_rebias(h_obj, false, thread);//撤销偏向锁，如果该偏向锁正在使用，则会被升级成轻量级锁
  ...
  markOop mark = ReadStableMark (obj) ;//获取对象头，如果正在锁膨胀，则通过自旋，yeild，park等方式等待锁膨胀完成

  // Uncontended case, header points to stack
  if (mark->has_locker()) {//如果是轻量级锁状态，则判断对象头包含的locker是否属于当前线程
    return thread->is_lock_owned((address)mark->locker());
  }
  // Contended case, header points to ObjectMonitor (tagged pointer)
  if (mark->has_monitor()) {//如果是重量级锁，则判断owner属性是否是当前线程指针
    ObjectMonitor* monitor = mark->monitor();
    return monitor->is_entered(thread) != 0 ;
  }
  // Unlocked case, header in place
  assert(mark->is_neutral(), "sanity check"); //无锁状态
  return false;
}
...
ObjectMonitor * ATTR ObjectSynchronizer::inflate (Thread * Self, oop object) {  //锁膨胀过程
  ...
  for (;;) {
      ...
      if (mark->has_locker()) {//大量的轻量级锁状态的线程cas操作，修改状态
...
void ObjectSynchronizer::deflate_idle_monitors() {  //锁降级 inflate
  assert(SafepointSynchronize::is_at_safepoint(), "must be at safepoint");
  ...
  } else for (ObjectMonitor* block = gBlockList; block != NULL; block = next(block)) { //遍历所有monitor
