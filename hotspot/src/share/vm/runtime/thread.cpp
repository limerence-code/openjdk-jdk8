...
void Thread::set_priority(Thread* thread, ThreadPriority priority) {
  ...
  // Can return an error!
  (void)os::set_priority(thread, priority); //设置线程优先级
}
...
bool Thread::is_interrupted(Thread* thread, bool clear_interrupted) {
  ...
  return os::is_interrupted(thread, clear_interrupted); //返回当前线程是否已经被中断，如果是且clear_interrupted为true，则将其中断标识清除掉
}
...
// Creates the initial Thread 创建main线程的方法
static oop create_initial_thread(Handle thread_group, JavaThread* thread, TRAPS) {
  ...
  Handle string = java_lang_String::create_from_str("main", CHECK_NULL);//初始化一个字符串main
  ...
  JavaCalls::call_special(&result, thread_oop,
                                   ...
                                   thread_group,
                                   string,
                                   CHECK_NULL);//调用Thread的构造方法，创建的Thread实例保存在thread_oop中
  return thread_oop();
}
...
void JavaThread::initialize() {
  ...
  set_jni_functions(jni_functions());//JNIEnv对象的初始化
...
#endif // INCLUDE_ALL_GCS
{
  initialize();//这个重载的构造函数特意为main线程准备的，初始化完了就ok了，不需要再创建内核线程，本身就在内核线程中
...
#endif // INCLUDE_ALL_GCS
{
  ...
  thr_type = entry_point == &compiler_thread_entry ? os::compiler_thread :   //是否编译线程，编译线程也是JavaThread，但是对应的os::ThreadType类型不同
                                                     os::java_thread;
  os::create_thread(this, thr_type, stack_sz);      //java中的Thread对应的内核线程，原生线程，底层通过pthread_create创建，创建成功后将其设置到Thread的_osthread属性中，然后等待其初始化完成
...
void JavaThread::run() {
  // initialize thread-local alloc buffer related fields //初始TLAB
  this->initialize_tlab();

  // used to test validitity of stack trace backs //记录栈基地址
  this->record_base_of_stack_pointer();

  // Record real stack base and size. //初始化OSThread的栈帧
  this->record_stack_base_and_size();

  // Initialize thread local storage; set before calling MutexLocker //初始化ThreadLocalStorage
  this->initialize_thread_local_storage();
  ...
  ThreadStateTransition::transition_and_fence(this, _thread_new, _thread_in_vm); //修改线程状态，注意此时会检查是否进入安全点同步，如果是则阻塞当前线程
...
static void ensure_join(JavaThread* thread) { //ensure_join是线程退出时调用
  // We do not need to grap the Threads_lock, since we are operating on ourself.
  Handle threadObj(thread, thread->threadObj());
  assert(threadObj.not_null(), "java thread object must exist");
  ObjectLocker lock(threadObj, thread); //创建ObjectLocker（实际就是加锁，构造函数加锁，析构函数解锁，具体就是在ObjectLocker对象生命周期结束的时候解锁）
  ...
  lock.notify_all(thread);  //唤醒所有在该thread oop关联的重量级锁上等待的线程
  // Ignore pending exception (ThreadDeath), since we are exiting anyway
  thread->clear_pending_exception(); //清除待处理异常（走到这个地方ObjectLocker生命周期结束，自动调用析构函数解锁）
}


// For any new cleanup additions, please check to see if they need to be applied to
// cleanup_failed_attach_current_thread as well.
void JavaThread::exit(bool destroy_vm, ExitType exit_type) {  //正常退出或者异常后退出都是执行这个方法
  ...
  if (!destroy_vm || JDK_Version::is_jdk12x_version()) {
    ...
    if (uncaught_exception.not_null()) {       //如果有未捕获异常
      Handle group(this, java_lang_Thread::threadGroup(threadObj()));
      {
        ...
        LinkResolver::resolve_virtual_call(callinfo, threadObj, recvrKlass, thread_klass,
                                           ...
                                           KlassHandle(), false, false, THREAD);  //这里获取一个类似方法集合的数据结构
        CLEAR_PENDING_EXCEPTION;
        methodHandle method = callinfo.selected_method();  //找到dispatch方法
        if (method.not_null()) {
          JavaValue result(T_VOID);
          JavaCalls::call_virtual(&result,
                                  threadObj, thread_klass,
                                  vmSymbols::dispatchUncaughtException_name(),   //jvm处理异常实际是在这个地方触发然后调用java中的方法
                                  vmSymbols::throwable_void_signature(),
                                  uncaught_exception,
                                  THREAD);
        } else {
          KlassHandle thread_group(THREAD, SystemDictionary::ThreadGroup_klass()); //如果是空，则调用线程组的处理异常方法
          JavaValue result(T_VOID);
          JavaCalls::call_virtual(&result,
                                  group, thread_group,
  ...
  Threads::remove(this); //从活动线程链表中移除
}
...
void JavaThread::prepare(jobject jni_thread, ThreadPriority prio) {//prio默认是NoPriority:-1
...
: JavaThread(&compiler_thread_entry) {//这里的compiler_thread_entry是一个方法指针，是该线程启动后自动执行的方法
  _env   = NULL;
  _log   = NULL;
  _task  = NULL;
  _queue = queue;
...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
  ...
  extern void JDK_Version_init();//预初始化，可以由特定平台按需覆盖定义，默认为空实现
  ...
  Arguments::process_sun_java_launcher_properties(args); //处理启动器相关参数，如-Dsun.java.launcher

  // Initialize the os module before using TLS
  os::init(); //OS模块的初始化，包含设置内存页大小，获取物理内存大小，锁的初始化

  // Initialize system properties.     //初始化系统属性，所有的属性通过Arguments类的静态属性_system_properties保存，该属性是一个SystemProperty类的指针
  Arguments::init_system_properties(); //每个属性对应一个SystemProperty实例，通过链表的形式关联起来

  // So that JDK version can be used as a discrimintor when parsing arguments
  JDK_Version_init(); //初始JDK_Version类的静态变量_current，获取当前JDK的版本信息，如_major主版本号，_minor次版本号，_update更新时间等
  ...
  // Initialize the os module after parsing the args //OS模块的二次初始化，包含线程锁的初始，polling_page和mem_serialize_page分配内存，信号处理初始化，设置线程栈大小，libpthread初始化
  jint os_init_2_result = os::init_2();    //设置线程锁，线程优先级策略的初始化
  if (os_init_2_result != JNI_OK) return os_init_2_result;

  jint adjust_after_os_result = Arguments::adjust_after_os();//UseNUMA时额外的参数设置
  ...
  if (Arguments::init_agents_at_startup()) { // 根据-agentlib/-agentpath and 转换后 -Xrun参数创建agents
  ...
  // Attach the main thread to this os thread //创建一个新的JavaThread并设置相关属性，注意这里并未创建一个新的线程，而是将当前线程同JavaThread对象关联起来，通过JavaThread管理相关属性
  JavaThread* main_thread = new JavaThread();     //main线程在这里创建的，但是如何和内核线程对应呢？？
  ...
  if (!main_thread->set_as_starting_thread()) {     //该方法内部实现关联
  ...
  ObjectMonitor::Initialize() ;   //ObjectMonitor初始化

  // Second phase of bootstrapping, VM is about entering multi-thread mode
  MemTracker::bootstrap_multi_thread();

  // Initialize global modules
  jint status = init_globals();//全局模块初始化，如Management模块，Bytecodes模块，ClassLoader模块，CodeCache模块，StubRoutines模块，Universe模块，Interpreter模块等
  if (status != JNI_OK) {
    delete main_thread;
    *canTryAgain = false; // don't let caller call JNI_CreateJavaVM again
    return status;
  ...
  JvmtiExport::transition_pending_onload_raw_monitors(); //将加载时创建的raw monitors转换成真正的raw monitor

  // Fully start NMT
  MemTracker::start();

  // Create the VMThread
  { TraceTime timer("Start VMThread", TraceStartupTime); // 创建VMThread
    VMThread::create();
  ...
  if (VerifyDuringStartup) {
    ...
    VM_Verify verify_op;
    VMThread::execute(&verify_op);//验证VMThread的状态
  }
  ...
  JvmtiExport::post_vm_start();  //通知JVMTI agents虚机已启动
  ...
  {//加载核心Java类文件
    ...
    main_thread->set_threadObj(thread_object); //将main_thread同Java中的thread_object关联起来
    // Set thread status to running since main thread has
    // been started and running.
    java_lang_Thread::set_thread_status(thread_object,
                                        java_lang_Thread::RUNNABLE);// 设置thread_object的线程状态
  ...
  reset_vm_info_property(CHECK_0);//将java.vm.info设置成nojit

  quicken_jni_functions();//初始化JNIEnv中GetField相关接口
  ...
  Management::record_vm_init_completed(); // 记录初始化完成时间
  ...
  SystemDictionary::compute_java_system_loader(THREAD); // 完成SystemClassLoader的加载
  if (HAS_PENDING_EXCEPTION) {
  ...
  if (UseConcMarkSweepGC || UseG1GC) { //并发标记的初始化，会创建一个新的执行标记的线程
  ...
  // Always call even when there are not JVMTI environments yet, since environments
  // may be attached late and JVMTI must track phases of VM execution
  JvmtiExport::enter_live_phase(); //JVMTI打标，标识JVM进入可用状态

  // Signal Dispatcher needs to be started before VMInit event is posted
  os::signal_init(); //创建一个新的线程处理信号

  // Start Attach Listener if +StartAttachListener or it can't be started lazily
  if (!DisableAttachMechanism) {
    AttachListener::vm_start(); //移除.java_pid文件，准备好初始化
    if (StartAttachListener || AttachListener::init_at_startup()) {
      AttachListener::init(); //初始化
    }
  }
  ...
  if (!EagerXrunInit && Arguments::init_libraries_at_startup()) {
    create_vm_init_libraries(); //加载-Xrun执行的agent 库
  }

  // Notify JVMTI agents that VM initialization is complete - nop if no agents.
  JvmtiExport::post_vm_initialized(); //通知JVMTI JVM初始化完成
  ...
  if (CleanChunkPoolAsync) {
    Chunk::start_chunk_pool_cleaner_task();//开启一个新的线程用于清理ChunkPool
  }

  // initialize compiler(s) // c1 和 c2编译器初始化，会创建编译线程
#if defined(COMPILER1) || defined(COMPILER2) || defined(SHARK)
  CompileBroker::compilation_init();
#endif

  if (EnableInvokeDynamic) { //加载动态代理相关类
...
#if INCLUDE_MANAGEMENT
  Management::initialize(THREAD);  //加载Management相关类
#endif // INCLUDE_MANAGEMENT
...
//初始化监控任务线程
  ...
  if (JDK_Version::current().post_vm_init_hook_enabled()) {
    call_postVMInitHook(THREAD);//回调代码中的钩子函数，通知其JVM初始化完成
...

  {
      ...
      WatcherThread::make_startable();// 确保WatcherThread可以通过start()的形式启动
      // Start up the WatcherThread if there are any periodic tasks
      // NOTE:  All PeriodicTasks should be registered by now. If they
      //   aren't, late joiners might appear to start slowly (we might
      //   take a while to process their first tick).
      if (PeriodicTask::num_tasks() > 0) {// 如果有任何周期的监控任务则启动监控线程
          WatcherThread::start();
      }
  }
...
bool Threads::destroy_vm() {
  ...
  VM_Exit::set_vm_exited();   //以本地代码运行的线程设置“vm exit”标记
...
void Threads::add(JavaThread* p, bool force_daemon) { //force_daemon默认为false
