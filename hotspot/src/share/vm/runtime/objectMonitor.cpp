...
static int Knob_PreSpin            = 10 ;      // 20-100 likely better   默认循环10次
...
bool ObjectMonitor::try_enter(Thread* THREAD) { // try_enter用于实现Unsafe类的tryMonitorEnter方法,会尝试获取锁，如果获取失败则直接返回false
  if (THREAD != _owner) {
    if (THREAD->is_lock_owned ((address)_owner)) { //如果该线程已经占有了该锁，该锁由轻量级锁膨胀而来
       assert(_recursions == 0, "internal state error");
       _owner = THREAD ;    //修改owner等属性
       _recursions = 1 ;
       OwnerIsThread = 1 ;
       return true;
    }
    if (Atomic::cmpxchg_ptr (THREAD, &_owner, NULL) != NULL) { //原子的设置owner属性，修改失败
      return false;
    }
    return true; //修改成功
  } else {
    _recursions++; //当前线程已经占有该锁，将记录嵌套加锁的计数器加1
    return true;
  }
}
...
void ATTR ObjectMonitor::enter(TRAPS) {//重量级锁入口
  ...
  if (cur == NULL) {//修改成功，返回（如果原值为NULL，则修改为新值，并且返回原值。否则，原值不是null，不做更新，并返回新值）
  ...
  if (cur == Self) {//如果是重入的，则_recursions+1（cas没做更新，直接返回新值）
  ...
  // We've encountered genuine contention. 到了这里，表示真的遇到了锁竞争，开始真正处理竞争情况
  ...
  if (Knob_SpinEarly && TrySpin (Self) > 0) { //这里会自旋操作（默认10次）
     assert (_owner == Self      , "invariant") ;
     assert (_recursions == 0    , "invariant") ;
  ...
  { // Change java thread status to indicate blocked on monitor enter.
    ...
    // TODO-FIXME: change the following for(;;) loop to straight-line code.  //cas及自适应自旋失败，此时就要入队挂起了
    for (;;) {
      jt->set_suspend_equivalent();
      // cleared by handle_special_suspend_equivalent_condition()
      // or java_suspend_self()

      EnterI (THREAD) ;//monitor竞争失败的线程，通过自旋执行ObjectMonitor::EnterI方法等待锁的释放
...
int ObjectMonitor::TryLock (Thread * Self) {
   for (;;) {
      ...
      if (Atomic::cmpxchg_ptr (Self, &_owner, NULL) == NULL) {//所谓抢锁就是把owner指向当前线程
...
void ATTR ObjectMonitor::EnterI (TRAPS) {  //这里就要入队挂起了
    Thread * Self = THREAD ;
    assert (Self->is_Java_thread(), "invariant") ;
    assert (((JavaThread *) Self)->thread_state() == _thread_blocked   , "invariant") ;

    // Try the lock - TATAS
    if (TryLock (Self) > 0) {//再尝试cas一波，还不死心
        assert (_succ != Self              , "invariant") ;
        assert (_owner == Self             , "invariant") ;
    ...
    if (TrySpin (Self) > 0) {//此处又有自旋获取锁的操作，实在是特么不想挂起啊
        assert (_owner == Self        , "invariant") ;
        assert (_succ != Self         , "invariant") ;
        assert (_Responsible != Self  , "invariant") ;
        return ;
    ...
    node.TState  = ObjectWaiter::TS_CXQ ;  //设置下状态为：TS_CXQ
    ...
    for (;;) {//循环，保证将node插入队列
        ...
        if (Atomic::cmpxchg_ptr (&node, &_cxq, nxt) == nxt) break ;//这里可能会有多个线程争抢塞入头节点。CAS修改_cxq指向node

        // Interference - the CAS failed because _cxq changed.  Just retry.
        // As an optional optimization we retry the lock.
        if (TryLock (Self) > 0) {//我再默默的TryLock一下，真的是不想挂起呀！如果成功return，此时不用进队列了
            assert (_succ != Self         , "invariant") ;
            assert (_owner == Self        , "invariant") ;
            assert (_Responsible != Self  , "invariant") ;
            return ;
    ...
    for (;;) {   //这里开始真正要阻塞了
        ...
        if (_Responsible == Self || (SyncFlags & 1)) {
            ...
            Self->_ParkEvent->park ((jlong) RecheckInterval) ;//park底层会调用mutex（进入内核态）
            // Increase the RecheckInterval, but clamp the value.
            RecheckInterval *= 8 ;
            if (RecheckInterval > 1000) RecheckInterval = 1000 ;
        } else {
            TEVENT (Inflated enter - park UNTIMED) ;
            Self->_ParkEvent->park() ;//park底层会调用mutex（进入内核态）终于挂起了，线程卡死在这里等待唤醒
        }

        if (TryLock(Self) > 0) break ;//这段要注意，此时是唤醒之后执行的，唤醒后依然要抢锁
...
void ATTR ObjectMonitor::ReenterI (Thread * Self, ObjectWaiter * SelfNode) { //ReenterI和EnterI的逻辑基本相同，用于获取对象锁
    assert (Self != NULL                , "invariant") ;
    assert (SelfNode != NULL            , "invariant") ;
    assert (SelfNode->_thread == Self   , "invariant") ;
    assert (_waiters > 0                , "invariant") ;
    assert (((oop)(object()))->mark() == markOopDesc::encode(this) , "invariant") ;//校验目标对象的对象头就是当前ObjectMonitor的指针
    assert (((JavaThread *)Self)->thread_state() != _thread_blocked, "invariant") ;
    JavaThread * jt = (JavaThread *) Self ;
    ...
    for (;;) {
        ...
        guarantee (v == ObjectWaiter::TS_ENTER || v == ObjectWaiter::TS_CXQ, "invariant") ;//校验状态
        assert    (_owner != Self, "invariant") ;

        if (TryLock (Self) > 0) break ;//尝试获取锁
        if (TrySpin (Self) > 0) break ;//尝试自旋获取锁
        ...
        {
           OSThreadContendState osts(Self->osthread()); //修改线程状态
           ThreadBlockInVM tbivm(jt);

           // cleared by handle_special_suspend_equivalent_condition()
           // or java_suspend_self()
           jt->set_suspend_equivalent();
           if (SyncFlags & 1) {            //SyncFlags默认是0
           ...
           for (;;) {
              if (!ExitSuspendEquivalent (jt)) break ;//ExitSuspendEquivalent默认返回false
    ...
    assert (_owner == Self, "invariant") ;  //for循环结束，已经获取了锁
    assert (((oop)(object()))->mark() == markOopDesc::encode(this), "invariant") ;
    UnlinkAfterAcquire (Self, SelfNode) ; //从链表中移除
    if (_succ == Self) _succ = NULL ;
    assert (_succ != Self, "invariant") ;
    SelfNode->TState = ObjectWaiter::TS_RUN ; //修改状态为TS_RUN
...
void ATTR ObjectMonitor::exit(bool not_suspended, TRAPS) { //exit用于释放锁，即将owner属性置为NULL
   Thread * Self = THREAD ; //默认配置下会通过unpark唤醒_EntryList链表头部节点对应的等待线程
   if (THREAD != _owner) {
     if (THREAD->is_lock_owned((address) _owner)) { //如果owner位于当前线程调用栈帧，说明该锁是轻量级锁膨胀来的
       // Transmute _owner from a BasicLock pointer to a Thread address.
       // We don't need to hold _mutex for this transition.
       // Non-null to Non-null is safe as long as all readers can
       // tolerate either flavor.
       assert (_recursions == 0, "invariant") ;
       _owner = THREAD ; //修改owner属性
       _recursions = 0 ;
       OwnerIsThread = 1 ;
     } else { //其他线程占用该锁，直接返回
   ...
   if (_recursions != 0) { //不等于0说明是嵌套加锁（重入的），将_recursions减1即可返回
   ...
   if ((SyncFlags & 4) == 0) { // SyncFlags默认值是0
      _Responsible = NULL ;
   }
   ...
   for (;;) {//此时解锁的线程还会唤醒之前等待的线程，这里有好几种模式
      assert (THREAD == _owner, "invariant") ;


      if (Knob_ExitPolicy == 0) { //Knob_ExitPolicy默认值是0
         ...
         OrderAccess::release_store_ptr (&_owner, NULL) ;   // drop the lock //将_owner属性置为NULL，释放锁，如果某个线程正在自旋抢占该锁，则会抢占成功.即这种策略会优先保证通过自旋抢占锁的线程获取锁，而其他处于等待队列中的线程则靠后
         OrderAccess::storeload() ;                         // See if we need to wake a successor  //让修改立即生效
         if ((intptr_t(_EntryList)|intptr_t(_cxq)) == 0 || _succ != NULL) {
            TEVENT (Inflated exit - simple egress) ; //如果_EntryList或者cxq链表都是空的，则直接返回
            return ;
         }
         ...
         //
         if (Atomic::cmpxchg_ptr (THREAD, &_owner, NULL) != NULL) { //如果_EntryList或者cxq链表不是空的，则原子的设置owner属性为当前线程，尝试抢占锁
            return ; //抢占失败则返回，等占用该锁的线程释放后再处理链表中的等待线程
         }
         ...
         if ((intptr_t(_EntryList)|intptr_t(_cxq)) == 0 || _succ != NULL) {
            ...
            if (Atomic::cmpxchg_ptr (THREAD, &_owner, NULL) != NULL) { //有可能cxq插入了一个新节点，导致上面的if不成立，需要重新获取锁
               TEVENT (Inflated exit - reacquired succeeded) ;
               return ;
            }
            TEVENT (Inflated exit - reacquired failed) ;
         } else {//如果_EntryList或者cxq链表不是空的则不释放锁，避免二次抢占锁，即优先处理等待队列中的线程
            TEVENT (Inflated exit - complex egress) ;
         }
      }
      ...
      ObjectWaiter * w = NULL ;
      int QMode = Knob_QMode ; //Knob_QMode的默认值是0

      if (QMode == 2 && _cxq != NULL) {//直接唤醒cxq头部线程，绕过entrylist
          ...
          ExitEpilog (Self, w) ;//通过unpark唤醒cxq对应的线程，唤醒后会将cxq从链表中移除
          return ; //模式2这里就返回了
      }
      ...
      if (QMode == 3 && _cxq != NULL) { 
          ...
          // Drain _cxq into EntryList - bulk transfer.  //批量迁移
          // First, detach _cxq.
          // The following loop is tantamount to: w = swap (&cxq, NULL)
          w = _cxq ;      
          for (;;) {                     //将_cxq原子的置为NULL，如果失败则重新更新，重新尝试直到成功为止.
             assert (w != NULL, "Invariant") ; //置为NULL后，如果有新的节点插入进来就形成了一个新的cxq链表
          ...
          ObjectWaiter * q = NULL ;
          ObjectWaiter * p ;
          for (p = w ; p != NULL ; p = p->_next) {                        //cxq队列中的节点全部在w中
              guarantee (p->TState == ObjectWaiter::TS_CXQ, "Invariant") ;
              p->TState = ObjectWaiter::TS_ENTER ;            //遍历cxq中的所有节点，将其置为TS_ENTER
              p->_prev = q ;
              q = p ;
          }                                                   //以上将cxq变成双向链表

          // Append the RATs to the EntryList             //将cxq链表中的元素插入到_EntryList链表的尾部
          // TODO: organize EntryList as a CDLL so we can locate the tail in constant-time.
          ObjectWaiter * Tail ;
          for (Tail = _EntryList ; Tail != NULL && Tail->_next != NULL ; Tail = Tail->_next) ;//遍历_EntryList找到末尾元素，将w插入到后面
          if (Tail == NULL) {
              _EntryList = w ;
          } else {
              Tail->_next = w ;    //entrylist---->w(cxq)  尾插
              w->_prev = Tail ;
          }
      ...
      if (QMode == 4 && _cxq != NULL) {
          ...
          // First, detach _cxq.
          // The following loop is tantamount to: w = swap (&cxq, NULL)
          w = _cxq ;   //将cxq链表中的元素插入到_EntryList链表的头部
          for (;;) {
             assert (w != NULL, "Invariant") ;                   //参数：新值，目标地址，原值
             ObjectWaiter * u = (ObjectWaiter *) Atomic::cmpxchg_ptr (NULL, &_cxq, w) ;//将_cxq原子的置为NULL，如果失败则更新w，并返回w。重新尝试直到成功为止
             if (u == w) break ;
             w = u ;
          }
          assert (w != NULL              , "invariant") ;
          ...
          for (p = w ; p != NULL ; p = p->_next) { 
              ...
              p->TState = ObjectWaiter::TS_ENTER ;  //遍历cxq中的所有节点，将其置为TS_ENTER
              p->_prev = q ;
              q = p ;      //这里也是双向链表，q最后指向_cxq尾部
          }

          // Prepend the RATs to the EntryList //插入到_EntryList的头部
          if (_EntryList != NULL) {
              q->_next = _EntryList ;    //q--->q---->entrylist
              _EntryList->_prev = q ;
          }
          _EntryList = w ;   //_EntryList再指向_cxq的头部（后面唤醒的就是最新的）

          // Fall thru into code that tries to wake a successor from EntryList
      }

      w = _EntryList  ;//模式0,3,4会执行到这里，_EntryList不为null
      if (w != NULL) {  //唤醒entrylist里的线程（3,4实际上是把要唤醒的线程先放入entrylist里，然后在这里唤醒）
          ...
          assert (w->TState == ObjectWaiter::TS_ENTER, "invariant") ;  //通过unpark唤醒w对应的线程，唤醒后会该线程会负责将w从EntryList链表中移除
          ExitEpilog (Self, w) ;//这里唤醒队头节点线程
          return ;
      }

      // If we find that both _cxq and EntryList are null then just
      // re-run the exit protocol from the top.
      w = _cxq ; //如果_EntryList为空
      ...
      for (;;) { //cxq不为NULL
          assert (w != NULL, "Invariant") ;
          ObjectWaiter * u = (ObjectWaiter *) Atomic::cmpxchg_ptr (NULL, &_cxq, w) ; //将cxq原子的修改为NULL
          if (u == w) break ;
          w = u ;
      }
      ...
      // we have faster access to the tail.  下面执行的是：cxq不为空，_EntryList为空的情况

      if (QMode == 1) {//遍历cxq中的元素将其加入到_EntryList中，注意顺序跟cxq中是反的（倒置）
         // QMode == 1 : drain cxq to EntryList, reversing order
         // We also reverse the order of the list.
         ObjectWaiter * s = NULL ;
         ObjectWaiter * t = w ;    //此时cxq已经赋值给w，w赋值给t了
         ObjectWaiter * u = NULL ;
         while (t != NULL) {
             guarantee (t->TState == ObjectWaiter::TS_CXQ, "invariant") ;
             t->TState = ObjectWaiter::TS_ENTER ;
             u = t->_next ;  //下面的操作是双向链表的倒置
             t->_prev = u ;  
             t->_next = s ;
             s = t;
             t = u ;
         }
         _EntryList  = s ; //_EntryList此时为倒置后的cxq队列
         assert (s != NULL, "invariant") ;
      } else { //遍历cxq中的元素将其加入到_EntryList中，注意此时cxq链表的头元素被赋值给EntryList
         // QMode == 0
         _EntryList = w ;
         ObjectWaiter * q = NULL ;
         ObjectWaiter * p ;
         for (p = w ; p != NULL ; p = p->_next) {//cxq中的元素是通过next属性串联起来的，prev属性没有，此处遍历加上prev属性.当EntryList头元素被移除了是取next属性作为EntryList
             guarantee (p->TState == ObjectWaiter::TS_CXQ, "Invariant") ;
...
void ObjectMonitor::ExitEpilog (Thread * Self, ObjectWaiter * Wakee) {
   ...
   _succ = Knob_SuccEnabled ? Wakee->_thread : NULL ;//Knob_SuccEnabled默认是1，succ表示很有可能占用该锁的线程
   ...
   // Drop the lock
   OrderAccess::release_store_ptr (&_owner, NULL) ;//将owner属性置为NULL
   ...
   Trigger->unpark() ;   //唤醒目标线程

   // Maintain stats and report events to JVMTI
   if (ObjectMonitor::_sync_Parks != NULL) {
      ObjectMonitor::_sync_Parks->inc() ; //增加计数
   }
}
...
intptr_t ObjectMonitor::complete_exit(TRAPS) { //complete_exit用于释放目标锁，在嵌套加锁的情形下只需要调用complete_exit一次即可，如果是exit则需要调用多次
   Thread * const Self = THREAD;
   ...
   _recursions = 0;        // set the recursion level to be 0 //_recursions置为0，即嵌套加锁的情形下不需要多次调用exit了
   exit (true, Self) ;           // exit the monitor //释放该锁
...
void ObjectMonitor::wait(jlong millis, bool interruptible, TRAPS) {//创建一个ObjectWaiter并加入到链表中,然后释放占有的锁,让当前线程休眠
   Thread * const Self = THREAD ;                          //当当前线程因为等待超时，被中断或者被其他线程唤醒时就再次抢占锁，
   assert(Self->is_Java_thread(), "Must be Java thread!");  //抢占逻辑就是之前的enter方法，抢占成功后wait方法退出
   JavaThread *jt = (JavaThread *)THREAD;

   DeferredInitialize () ; //初始化配置，如果已经初始化则返回

   // Throw IMSX or IEX.
   CHECK_OWNER();  //检查当前线程是否获取了锁，如果没有则抛出异常

   EventJavaMonitorWait event;

   // check for a pending interrupt
   if (interruptible && Thread::is_interrupted(Self, true) && !HAS_PENDING_EXCEPTION) { //如果线程被中断了且不是因为未处理异常导致的
     // post monitor waited event.  Note that this is past-tense, we are done waiting.
     if (JvmtiExport::should_post_monitor_waited()) {  //发布JVMTI事件
     ...
     THROW(vmSymbols::java_lang_InterruptedException()); //抛出异常
     return ;
   }
   ...
   assert (Self->_Stalled == 0, "invariant") ;
   Self->_Stalled = intptr_t(this) ;   //设置属性，记录当前线程等待的ObjectMonitor
   jt->set_current_waiting_monitor(this);

   // create a node to be put into the queue
   // Critically, after we reset() the event but prior to park(), we must check
   // for a pending interrupt.
   ObjectWaiter node(Self);         //创建ObjectWaiter，将其状态置为TS_WAIT
   ...
   Thread::SpinAcquire (&_WaitSetLock, "WaitSet - add") ;  //获取操作ObjectWaiter链表的锁_WaitSetLock
   AddWaiter (&node) ;                                    //将当前节点插入到ObjectWaiter链表中
   Thread::SpinRelease (&_WaitSetLock) ;                  //释放锁

   if ((SyncFlags & 4) == 0) {  //SyncFlags默认为0
      _Responsible = NULL ;
   }
   intptr_t save = _recursions; // record the old recursion count
   _waiters++;                  // increment the number of waiters   //等待的线程数加1
   _recursions = 0;             // set the recursion level to be 1
   exit (true, Self) ;                    // exit the monitor       //释放该锁(wait释放锁的证据)
   guarantee (_owner != Self, "invariant") ;
   ...
   { // State transition wrappers
     ...
     OSThreadWaitState osts(osthread, true);//修改线程状态为OBJECT_WAIT
     {
       ThreadBlockInVM tbivm(jt);  //修改线程状态从_thread_in_vm到_thread_blocked
       ...
       if (node._notified == 0) {     //_notified为0表示没有其他线程唤醒
         if (millis <= 0) {           //将当前线程park，让其处于休眠状态,线程在此挂起
            Self->_ParkEvent->park () ;
         } else {
            ret = Self->_ParkEvent->park (millis) ;//线程挂起有时间限制，时间一到自动唤醒
         }
       }
              //以下是唤醒后，删除waitset中的节点，然后重新抢锁（此时是重量级锁）
       // were we externally suspended while we were waiting?
       if (ExitSuspendEquivalent (jt)) {    //当前线程从park状态被唤醒了,ExitSuspendEquivalent默认返回false
     ...
     if (node.TState == ObjectWaiter::TS_WAIT) { //如果是线程被中断或者等待超时则状态是TS_WAIT，如果是被nofity唤醒的则应该是TS_RUN
         Thread::SpinAcquire (&_WaitSetLock, "WaitSet - unlink") ; //获取锁
         if (node.TState == ObjectWaiter::TS_WAIT) {
            DequeueSpecificWaiter (&node) ;       // unlink from WaitSet  //如果是TS_WAIT，则将其从链表中移除
            assert(node._notified == 0, "invariant");
            node.TState = ObjectWaiter::TS_RUN ;  //将状态置为TS_RUN
         }
         Thread::SpinRelease (&_WaitSetLock) ;  //释放锁
     }
     ...
     if (v == ObjectWaiter::TS_RUN) {
         enter (Self) ;              //重新获取该锁
     } else {                       //该ObjectWaiter已经被唤醒了，但是等待获取锁的时候线程被中断了
   ...
   if (!WasNotified) {                   //如果不是因为notify被唤醒（一定是先被中断唤醒后才走到这里和sleep机制不一样）
     // no, it could be timeout or Thread.interrupt() or both
     // check for interrupt event, otherwise it is timeout
     if (interruptible && Thread::is_interrupted(Self, true) && !HAS_PENDING_EXCEPTION) { // 可能因为等待超时或者Thread.interrupt()被唤醒
       TEVENT (Wait - throw IEX from epilog) ;
       THROW(vmSymbols::java_lang_InterruptedException()); //如果线程中断则抛出异常
     }
   }
...
void ObjectMonitor::notify(TRAPS) { //唤醒WaitSet链表头对应的线程，即最早加入到该链表的等待线程
  CHECK_OWNER();           //检查当前线程是否占用该锁，如果没有抛出异常
  if (_WaitSet == NULL) {   //如果没有等待的线程则退出
     TEVENT (Empty-Notify) ;
     return ;
  }
  DTRACE_MONITOR_PROBE(notify, this, object(), THREAD);

  int Policy = Knob_MoveNotifyee ; //Knob_MoveNotifyee属性默认是2

  Thread::SpinAcquire (&_WaitSetLock, "WaitSet - notify") ; //获取锁
  ObjectWaiter * iterator = DequeueWaiter() ;        //将链表头元素移除并返回
  if (iterator != NULL) {
     ...
     iterator->_notified = 1 ;//_notified置为1表示该ObjectWaiter被唤醒了
     Thread * Self = THREAD;
     iterator->_notifier_tid = Self->osthread()->thread_id(); //记录当前线程ID
     ...
     if (Policy == 0) {       // prepend to EntryList //根据不同的策略执行不同的处理
         if (List == NULL) {       //将iterator插入到_EntryList头元素的前面
             iterator->_next = iterator->_prev = NULL ;
             _EntryList = iterator ;
         } else {
             List->_prev = iterator ;    //EntryList是双向链表（非环形循环的）
             iterator->_next = List ;
             iterator->_prev = NULL ;
             _EntryList = iterator ;
        }
     } else
     if (Policy == 1) {      // append to EntryList //根据不同的策略执行不同的处理
         if (List == NULL) {        //将iterator插入到_EntryList链表的末尾
             iterator->_next = iterator->_prev = NULL ;
             _EntryList = iterator ;
         } else {
            ...
            ObjectWaiter * Tail ;
            for (Tail = List ; Tail->_next != NULL ; Tail = Tail->_next) ;//不断遍历找到链表最后一个元素
     ...
     if (Policy == 2) {      // prepend to cxq    //将iterator插入到_cxq头元素的前面
         ...
         if (List == NULL) {
            ...
            for (;;) { //cas循环，在cxq头部成功插入为止
                ObjectWaiter * Front = _cxq ;
                iterator->_next = Front ;
                if (Atomic::cmpxchg_ptr (iterator, &_cxq, Front) == Front) {//cxq队列的操作一定是cas的
                    break ;
                }
            }
         }
     ...
     if (Policy == 3) {      // append to cxq     //将iterator插入到_cxq链表末尾的后面
        ...
        for (;;) {
            ...
            if (Tail == NULL) {
                iterator->_next = NULL ;
                if (Atomic::cmpxchg_ptr (iterator, &_cxq, NULL) == NULL) {  //cas插入到尾部
                   break ;
                }
            } else {
                while (Tail->_next != NULL) Tail = Tail->_next ;//往后遍历找到最后一个元素
                Tail->_next = iterator ;
                iterator->_prev = Tail ;
                iterator->_next = NULL ;
                break ;
     ...
     } else {  //Policy为其他值时立即将等待的线程直接unpark唤醒
     ...
     if (Policy < 4) {
       iterator->wait_reenter_begin(this);//修改线程状态，记录锁竞争开始
     }
  ...
  if (iterator != NULL && ObjectMonitor::_sync_Notifications != NULL) {
     ObjectMonitor::_sync_Notifications->inc() ;  //增加计数
  }
}


void ObjectMonitor::notifyAll(TRAPS) { //相比notify的实现就是增加了一个for循环，会不断的从_WaitSet链表中移除头元素，然后执行notify的处理逻辑，直到_WaitSet链表为空退出循环
  CHECK_OWNER();     //检查当前线程是否占用该锁，如果没有抛出异常
  ObjectWaiter* iterator;
  if (_WaitSet == NULL) {
      TEVENT (Empty-NotifyAll) ;//如果没有等待的线程则退出
      return ;
  }
  ...
  int Policy = Knob_MoveNotifyee ;//Knob_MoveNotifyee属性默认是2
  int Tally = 0 ;
  Thread::SpinAcquire (&_WaitSetLock, "WaitSet - notifyall") ; //获取锁

  for (;;) { //if变成for循环
     iterator = DequeueWaiter () ; //获取头部元素，头部节点为最早加入到链表中的节点
     if (iterator == NULL) break ;  //如果为空则终止循环
     ...
     iterator->_notifier_tid = Self->osthread()->thread_id(); //记录当前线程ID
     if (Policy != 4) {
        iterator->TState = ObjectWaiter::TS_ENTER ;  //将状态置为TS_ENTER
     }

     ObjectWaiter * List = _EntryList ; //根据不同的策略执行不同的处理
     if (List != NULL) {
        assert (List->_prev == NULL, "invariant") ;
        assert (List->TState == ObjectWaiter::TS_ENTER, "invariant") ;
        assert (List != iterator, "invariant") ;
  ...
  if (Tally != 0 && ObjectMonitor::_sync_Notifications != NULL) {
     ObjectMonitor::_sync_Notifications->inc(Tally) ; //增加计数
  }
}
...
//默认配置下自旋的次数是会自适应调整的，可以通过参数指定自旋固定的次数，注意在自旋的过程中会判断是否进入安全点同步，如果是则终止自旋。
    ...
    int ctr = Knob_FixedSpin ; //固定次数的自旋
    if (ctr != 0) {
        while (--ctr >= 0) {
    ...
    for (ctr = Knob_PreSpin + 1; --ctr >= 0 ; ) {//上次自旋次数，Knob_PreSpin默认是10次
      if (TryLock(Self) > 0) {//每次尝试获取锁
...
inline void ObjectMonitor::AddWaiter(ObjectWaiter* node) { //该方法用于将目标ObjectWaiter加入到双向循环链表中
  assert(node != NULL, "should not dequeue NULL node");
  assert(node->_prev == NULL, "node already in list");
  assert(node->_next == NULL, "node already in list");
  // put node at end of queue (circular doubly linked list)
  if (_WaitSet == NULL) {  //如果_WaitSet还是空的，当前节点就是第一个
    _WaitSet = node;
    node->_prev = node;
    node->_next = node;
  } else {              //如果_WaitSet不是空的，将其插入到head的prev节点上
    ObjectWaiter* head = _WaitSet ;
    ObjectWaiter* tail = head->_prev;
    assert(tail->_next == head, "invariant check");
    tail->_next = node;    //注意tail在初始状态下就是head，所以插入第二个节点时修改next属性，实际是修改head的next属性
    head->_prev = node;
    node->_next = head;
    node->_prev = tail;
  }
...
inline ObjectWaiter* ObjectMonitor::DequeueWaiter() { //DequeueWaiter用于移除链表头_WaitSet对应的节点
  // dequeue the very first waiter                //该节点是最早加入到链表的，即按照加入链表的先后顺序依次从链表中移除
  ObjectWaiter* waiter = _WaitSet;
  if (waiter) {
    DequeueSpecificWaiter(waiter); //用于移除指定节点，不一定是_WaitSet对应的节点
  }
  return waiter;
}

inline void ObjectMonitor::DequeueSpecificWaiter(ObjectWaiter* node) { //从waitset中移除要唤醒的节点
  assert(node != NULL, "should not dequeue NULL node");
  assert(node->_prev != NULL, "node already removed from list");
  assert(node->_next != NULL, "node already removed from list");
  // when the waiter has woken up because of interrupt,  //从_WaitSet中取出一个ObjectWaiter，实际就是取出_WaitSet
  // timeout or other spurious wake-up, dequeue the     //对应的head节点，该
  // waiter from waiting list                          //节点是最早加入到链表中的
  ObjectWaiter* next = node->_next;  //node指向链表头
  if (next == node) {               //_WaitSet只有一个节点
    assert(node->_prev == node, "invariant check");
    _WaitSet = NULL;
  } else {                         //将node从链表中移除
    ObjectWaiter* prev = node->_prev;        //移除的是双向链表对头第一个节点，也是最早进来的节点
    assert(prev->_next == node, "invariant check");
    assert(next->_prev == node, "invariant check");
    next->_prev = prev;
    prev->_next = next;
    if (_WaitSet == node) {       //如果移除的就是_WaitSet，将next置为_WaitSet
      _WaitSet = next;
    }
  }
  node->_next = NULL;        //相关属性置为null
  node->_prev = NULL;
}
