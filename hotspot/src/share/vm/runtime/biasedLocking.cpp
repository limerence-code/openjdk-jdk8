...
void BiasedLocking::init() {   //偏向锁初始化，jvm启动时调用，启动后4秒生效
  ...
  if (UseBiasedLocking) { //UseBiasedLocking默认为true，表示是否使用偏向锁
    if (BiasedLockingStartupDelay > 0) {//BiasedLockingStartupDelay表示BiasedLocking初始化时的延时，默认为4000，避免在JVM启动时初始化，可加快JVM的启动
...
static HeuristicsResult update_heuristics(oop o, bool allow_rebias) {//1、update_heuristics()方法会将类对象上revoke次数加1。启发式的方式决定要做哪种操作
  markOop mark = o->mark();
...
//控制撤销的次数
  ...
  if ((revocation_count >= BiasedLockingBulkRebiasThreshold) &&
      ...
      (cur_time - last_bulk_revocation_time >= BiasedLockingDecayTime)) {//在执行了一定时间之内，执行的撤销次数没有超过阈值，那么认为可以优先执行bulk rebias,因此将计数回归原始值
  ...
  if (revocation_count <= BiasedLockingBulkRevokeThreshold) {//计算执行撤销的次数(增加revoke次数)
    revocation_count = k->atomic_incr_biased_lock_revocation_count();
  }

  if (revocation_count == BiasedLockingBulkRevokeThreshold) {// 如果revoke次数等于BiasedLockingBulkRevokeThreshold(默认40)
    return HR_BULK_REVOKE; //达到执行bulk revoke的阈值，执行bulk revoke    return HR_BULK_REVOKE
  }

  if (revocation_count == BiasedLockingBulkRebiasThreshold) {// 如果revoke次数等于BiasedLockingBulkRebiasThreshold(默认20)
    return HR_BULK_REBIAS; //达到 bulk rebias的阈值，执行bulk rebias
  }

  return HR_SINGLE_REVOKE; //默认执行单次的撤销
}
...
//2、如果撤销次数等于BiasedLockingBulkRebiasThreshold(默认20)，则认为类对象还可以重偏向，因此要做以下操作(bulk rebias)
  if (TraceBiasedLocking) {
  ...
  if (bulk_rebias) {
    ...
    if (klass->prototype_header()->has_bias_pattern()) {
      ...
      for (JavaThread* thr = Threads::first(); thr != NULL; thr = thr->next()) {// 遍历所有线程的栈，找到所有类对象的实例，将它们Mark Word中的epoch值加1
...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj, bool attempt_rebias, TRAPS) {
  ...
  markOop mark = obj->mark();                           //2、获取对象头的Mark Word
  if (mark->is_biased_anonymously() && !attempt_rebias) { //3、没有线程获取了偏向锁（线程那栏为空），说明不处于偏向状态
    ...
    markOop unbiased_prototype = markOopDesc::prototype()->set_age(mark->age());//prootype本身构建的是 markOop( no_hash_in_place | no_lock_in_place )
    markOop res_mark = (markOop) Atomic::cmpxchg_ptr(unbiased_prototype, obj->mark_addr(), mark);//执行CAS,如果当前对象的mark没有变更，就换成 unbiased_prototype
    if (res_mark == biased_value) {//如果之前的和现在的一样，说明撤销成功，BIAS_REVOKED本身是一个枚举
      return BIAS_REVOKED;
    }
  } else if (mark->has_bias_pattern()) {  //4、已经偏向了
    Klass* k = obj->klass();//已经被线程偏向了，获取Klass对象，即类本身的头，obj则是它的实例
    ...
    if (!prototype_header->has_bias_pattern()) {
      ...
      // with it.                                //对象当前的偏向状态已经过期,并且是不可偏向的，直接设置成已经撤销偏向即可 
      markOop biased_value       = mark;
      markOop res_mark = (markOop) Atomic::cmpxchg_ptr(prototype_header, obj->mark_addr(), mark);
      assert(!(*(obj->mark_addr()))->has_bias_pattern(), "even if we raced, should still be revoked");
      return BIAS_REVOKED;
    } else if (prototype_header->bias_epoch() != mark->bias_epoch()) {
      // The epoch of this biasing has expired indicating that the //实例的epoch和类本身的epoch值不一样，说明它已经过期，
      // object is effectively unbiased. Depending on whether we need//也就是说这个对象当前处于未偏向但是可偏向的状态(rebiasable) 
      ...
      if (attempt_rebias) {
        assert(THREAD->is_Java_thread(), "");//执行rebias ,wait希望直接撤销
        markOop biased_value       = mark;
        ...
        markOop res_mark = (markOop) Atomic::cmpxchg_ptr(unbiased_prototype, obj->mark_addr(), mark);//CAS撤销偏向锁
        if (res_mark == biased_value) {//撤销了偏向
          return BIAS_REVOKED;
        }
      }
    }
  ...
  HeuristicsResult heuristics = update_heuristics(obj(), attempt_rebias);//5、没有执行偏向，通过启发式的方式决定到底是执行撤销还是执行rebias
  if (heuristics == HR_NOT_BIASED) {
    ...
    if (mark->biased_locker() == THREAD &&
      ...
      BiasedLocking::Condition cond = revoke_bias(obj(), false, false, (JavaThread*) THREAD);//撤销偏向
  ...
  VM_BulkRevokeBias bulk_revoke(&obj, (JavaThread*) THREAD,
                                (heuristics == HR_BULK_REBIAS),
                                attempt_rebias); //6、等到虚拟机运行到safepoint,实际就是执行VM_BulkRevokeBias的doit的 bulk_revoke_or_rebias_at_safepoint方法
...
void BiasedLocking::revoke_at_safepoint(Handle h_obj) {
  ...
  oop obj = h_obj();
  HeuristicsResult heuristics = update_heuristics(obj, false);//1、update_heuristics()方法会将类对象上revoke次数加1
  if (heuristics == HR_SINGLE_REVOKE) {
    revoke_bias(obj, false, false, NULL);  //开始撤销
  } else if ((heuristics == HR_BULK_REBIAS) ||
             (heuristics == HR_BULK_REVOKE)) {//2、如果撤销次数等于BiasedLockingBulkRebiasThreshold(默认20)，则认为类对象还可以重偏向，因此要做以下操作(bulk rebias)
