...
class JavaThread: public Thread {
 ...
 public:
  ...
  static JavaThread* thread_from_jni_environment(JNIEnv* env) {//每个JavaThread都有一个单独的JNIEnv实例，此处是根据env的地址和其在JavaThread中的属性偏移量倒推出JavaThread的地址
    JavaThread *thread_from_jni_env = (JavaThread*)((intptr_t)env - in_bytes(jni_environment_offset()));
    // Only return NULL if thread is off the thread list; starting to
    // exit should not return NULL.
    if (thread_from_jni_env->is_terminated()) {//如果JVM正在退出的过程中，则阻塞当前线程
...
class CompilerThread : public JavaThread { //表示一个专门执行后台编译的线程,比如这种线程在运行时会将字节码编译成本地代码
  friend class VMStructs;
 private:
