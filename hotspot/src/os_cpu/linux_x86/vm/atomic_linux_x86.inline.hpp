...
inline jint     Atomic::cmpxchg    (jint     exchange_value, volatile jint*     dest, jint     compare_value) {
  int mp = os::is_MP();//判断是否多核
  __asm__ volatile (LOCK_IF_MP(%4) "cmpxchgl %1,(%3)"    //加了lock 前缀之后，该指令具备了原子性
                    : "=a" (exchange_value)              //%0 表示返回值（返回逻辑：将rax寄存器的值放入exchange_value，然后将exchange_value返回）
