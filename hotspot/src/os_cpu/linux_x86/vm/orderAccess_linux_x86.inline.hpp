...
inline void OrderAccess::loadload()   { acquire(); }// 定义在java读之前在编译器阶段和CPU执行阶段的时候在acquire之后的所有的读写操作是不能越过acquire指令重排到acquire之前，可以让之后的所有的读都具有可见性
inline void OrderAccess::storestore() { release(); }// 定义在java写之后在编译器阶段和CPU执行阶段的时候在release之前的所有的读写操作是不能越过release指令重排到release之后。强制刷新到主内存
inline void OrderAccess::loadstore()  { acquire(); }
inline void OrderAccess::storeload()  { fence(); }//在编译器阶段和CPU执行阶段的时候在fence之前的所有操作都不能越过fence指令重排到fence之后。

inline void OrderAccess::acquire() {
  volatile intptr_t local_dummy;
#ifdef AMD64//将寄存器栈顶值复制给局部变量，保证了编译器不会重排序，这里没使用lfence指令，因为x86不会发生read read重排序,所以只需要保证编译器不会重排序指令即可
...
inline void OrderAccess::release() { //空操作
...
#ifdef AMD64//使用了lock前缀做内存屏障，add一个无用的操作，这样的方式比直接使用mfence指令效率高
    __asm__ volatile ("lock; addl $0,0(%%rsp)" : : : "cc", "memory");// volatile (禁止编译器对代码的优化)  //处理器
#else
    __asm__ volatile ("lock; addl $0,0(%%esp)" : : : "cc", "memory");// cc(寄存器) memory(内存)           //编译器
#endif
  }
}
