...
// Thread start routine for all newly created threads //所有线程创建完都会执行这个方法，而且会阻塞，直到线程状态为RUNNABLE为止
static void *java_start(Thread *thread) {
  ...
  thread->run();                             //此时线程状态已经是runnable了，于是执行run方法
...
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  ...
  if (os::Linux::supports_variable_stack_size()) {
    ...
    if (stack_size == 0) {
      stack_size = os::Linux::default_stack_size(thr_type);//默认1M
    ...
    stack_size = MAX2(stack_size, os::Linux::min_stack_allowed);//传入的值和linux允许的最小值（236k）中取一个大的
...
int os::sleep(Thread* thread, jlong millis, bool interruptible) {
  ...
  slp->reset() ;
  OrderAccess::fence() ;//这里强刷内存，保证可见性

  if (interruptible) {//如果是可中断的
    jlong prevtime = javaTimeNanos();

    for (;;) {//这里死循环检查中断
      if (os::is_interrupted(thread, true)) {//如果线程已被中断
        return OS_INTRPT;
      }
      ...
      if (newtime - prevtime < 0) {
        ...
        assert(!Linux::supports_monotonic_clock(), "time moving backwards");//linux不支持monotonic_clock
      } else {
...

      {
        ...
        JavaThread *jt = (JavaThread *) thread;
        ThreadBlockInVM tbivm(jt);//修改线程状态为_thread_blocked，等代码块退出将其恢复成_thread_in_vm，转换过程中会检查安全点
        OSThreadWaitState osts(jt->osthread(), false /* not Object.wait() */);//将OSThread的状态修改为CONDVAR_WAIT，代码块退出时恢复原来的状态

        jt->set_suspend_equivalent();
        // cleared by handle_special_suspend_equivalent_condition() or
        // java_suspend_self() via check_and_wait_while_suspended()

        slp->park(millis);//让当前线程休眠，如果线程被唤醒了则继续for循环

        // were we externally suspended while we were waiting?
        jt->check_and_wait_while_suspended();
      }
    }
  } else {//如果不能打断，除了不再返回OS_INTRPT以外，逻辑是完全相同的
    OSThreadWaitState osts(thread->osthread(), false /* not Object.wait() */);
    jlong prevtime = javaTimeNanos();

    for (;;) { //逻辑同上只是不需要检查目标线程是否中断，使用与sleep时间特别短的情形，如sleep(0)
...
void os::yield() {
  sched_yield();//sched_yield()是Linux的API，会让出当前线程的CPU占有权，然后把线程放到调度队列的尾端，即优先调度其他的线程,给他们分配CPU时间片
}
...
int os::java_to_os_priority[CriticalPriority + 1] = {
  19,              // 0 Entry should never be used
   ...
   4,              // 1 MinPriority  这里对应java中的优先级
   3,              // 2
   2,              // 3

   1,              // 4
   0,              // 5 NormPriority 比如java中优先级5对应OSThread的优先级就是0
  -1,              // 6
...
void os::interrupt(Thread* thread) {
  ...
  if (!osthread->interrupted()) {//中断只干两件事
    osthread->set_interrupted(true);//1、将interrupted标识置为true
    // More than one thread can get here with the same value of osthread,
    // resulting in multiple notifications.  We do, however, want the store
    // to interrupted() to be visible to other threads before we execute unpark().
    OrderAccess::fence();
    ParkEvent * const slp = thread->_SleepEvent ;//针对sleep的唤醒
    if (slp != NULL) slp->unpark() ;//2-1、唤醒在_SleepEvent上等待的线程，_SleepEvent用于实现线程sleep
  }

  // For JSR166. Unpark even if interrupt status already was set
  if (thread->is_Java_thread())//2-2、唤醒在parker上等待的线程，parker用于实现Unsafe的park和unpark方法
...
bool os::is_interrupted(Thread* thread, bool clear_interrupted) {
  ...
  bool interrupted = osthread->interrupted(); //获取其是否被中断

  if (interrupted && clear_interrupted) {//如果需要清除interrupted标识
    osthread->set_interrupted(false);  //清除被中断标识
...
jint os::init_2(void)
{
  ...
  os::Linux::min_stack_allowed = MAX2(os::Linux::min_stack_allowed,
            (size_t)(StackYellowPages+StackRedPages+StackShadowPages) * Linux::page_size() +
                    (2*BytesPerWord COMPILER2_PRESENT(+1)) * Linux::vm_default_page_size()); //236k
...
void os::PlatformEvent::park() {       // AKA "down()"
  ...
  if (v == 0) {
     ...
     int status = pthread_mutex_lock(_mutex);//获取mutex锁，阻塞调用（这里也有队列，操作系统的阻塞队列）
     assert_status(status == 0, status, "mutex_lock");
     guarantee (_nParked == 0, "invariant") ;
     ++ _nParked ;
     while (_Event < 0) {
        status = pthread_cond_wait(_cond, _mutex);//这里阻塞挂起，等待唤醒
    ...
    _Event = 0 ;
     status = pthread_mutex_unlock(_mutex);//唤醒之后解锁
...
int os::PlatformEvent::park(jlong millis) {  //sleep的真正实现是park
  guarantee (_nParked == 0, "invariant") ;
...
void Parker::park(bool isAbsolute, jlong time) {
  ...
  if (Atomic::xchg(0, &_counter) > 0) return;     //0和_counter比较，如果_counter>0,则将0放入_counter，返回，没阻塞住。此时_counter=0了（交换了）
  ...
  if (Thread::is_interrupted(thread, false)) {   //如果发生中断，也不会阻塞住，返回
    return;
  }
  ...
  if (Thread::is_interrupted(thread, false) || pthread_mutex_trylock(_mutex) != 0) {//如果此方法执行失败，说明unpark正在执行，此时没有释放mutex锁，直接返回（此方法返回true，表示该线程没有执行unpark或者已经执行完unpark）
    return;
  }

  int status ;
  if (_counter > 0)  { // no wait needed  走到这里表示unpark不仅执行完，而且已经执行完了，此时counter=1
    _counter = 0;
    status = pthread_mutex_unlock(_mutex);
  ...
  if (time == 0) {
    ...
    status = pthread_cond_wait (&_cond[_cur_index], _mutex) ; //真正的阻塞在这里
  } else {
    _cur_index = isAbsolute ? ABS_INDEX : REL_INDEX;
    status = os::Linux::safe_cond_timedwait (&_cond[_cur_index], _mutex, &absTime) ; //这里也是可以阻塞的
    if (status != 0 && WorkAroundNPTLTimedWaitHang) {
...
void Parker::unpark() {
  ...
  assert (status == 0, "invariant") ;
  s = _counter;   //猜测是0
  _counter = 1;     //唤醒时 _counter改为1
  if (s < 1) {    // _counter<1的时候唤醒线程
